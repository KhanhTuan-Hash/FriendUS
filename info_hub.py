...
class InformationHub:
    ...
    def _allocate_id(self) -> int:
        """Allocate a document ID, reusing deleted IDs first - O(1) amortized"""
        if self.deleted_ids:
            return self.deleted_ids.pop()
        else:
            id_val = self.next_id
            self.next_id += 1
            return 1000 * self.group_id + id_val
    ...
    def search(self, keyword: str) -> List[Information]:
        ...
        if keyword in self.title_index:
            title_matches.update(doc_id for doc_id, _ in self.title_index[keyword])

        content_matches = set()
        if keyword in self.content_index:
            content_matches.update(doc_id for doc_id, _ in self.content_index[keyword])
...
def main():
    hub = get_info_hub(1)
    # Add documents
    doc1 = hub.add_document(
        "Python Programming",
        "Python is a great programming language for beginners and experts alike."
    )
    doc2 = hub.add_document(
        "Machine Learning with Python",
        "Learn machine learning using Python and scikit-learn."
    )
    doc3 = hub.add_document(
        "Web Development",
        "Build web applications with Python using Django and Flask."
    )

    # Search - prioritizes title matches
    print("Search 'Python':")
    results = hub.search("python")
    for doc in results:
        print(f"- {doc.title}")

    print("\nSearch 'learning':")
    results = hub.search("learning")
    for doc in results:
        print(f"- {doc.title}")

    # Multiple keyword search
    print("\nSearch 'python machine learning':")
    results = hub.search_multiple(["python", "machine", "learning"])
    for doc in results:
        print(f"- {doc.title}")

    print(f"\nStats: {hub.get_stats()}")

    hub.delete_document(doc2)
    print(f"\nStats: {hub.get_stats()}")

    hub.add_document("Ticket link", "abcexample.com")
    print(f"\nStats: {hub.get_stats()}")
